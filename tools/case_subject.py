...
_AGENCY = re.compile(
    # ⚠️ 必须有「市场监管总局 / 市场监督管理总局」这两个**总局**写法：
    # 只列「…监督管理局」时总局名不被屏蔽，主体会被抽成
    # 「市场监管总局发布中山火炬集团有限公司」。
    # `{0,14}?`（而非 {2,14}?）：尾部「市场监管总局」本身就是完整机构名，
    # 要求前面至少 2 个汉字会让它匹配不上。
    r"[\u4e00-\u9fa5]{0,14}?(?:市场监督管理总局|市场监管总局|"
    r"市场监督管理局|市场监督管理所|市场监管管理局|"
...
_PREFIX_CUT = re.compile(
    ...
    r"处理|约谈|移送|用于|所属|直属|发布|上述|相关|有关|对|向|给|在|与|及|其|该|本|经|由)+)|"
    # 「拟吊销XX公司营业执照」「决定吊销XX公司…」——送达公告里描述处罚结果的动词
    r"^(?:拟|决定|依法|予以|给予)?(?:吊销|撤销|注销|没收|罚款)|"
    # 「由本局立案调查的XX公司…」「经核查的…」——定语式前缀
    r"^(?:由|经|本局)?(?:立案)?(?:调查|核查|检查|查明|认定|发现|查获|处理)的"
)

# 「本局于 2026 年 9 月 4 日依法对 XX 有限公司…作出…」这类句子开头的**时间+执法套话**。
# ⚠️ 只靠 _PREFIX_CUT 切不干净：它是单次替换，切掉句首的「本」之后就停在「局于 2026…」，
# 而「局于…」不在前缀词表里 → 主体名被抽成「局于2026年9月4日依法对青岛诺轩建筑材料有限公司」。
# 这里按「日期串 + 依法/对/向」整段切，再回到前缀词表清理残留。
_DATE_LEAD = re.compile(
    r"^.{0,14}?20\d{2}\s*年\s*\d{1,2}\s*月\s*\d{1,2}\s*日\s*(?:依法|依规|依据[^，。]{0,20})?\s*(?:对|向|给予|予以|作出|决定)?"
)
# 「局/公司/单位/机关」这类半截机构残片开头 → 直接砍掉
_HALF_ORG = re.compile(r"^(?:局|委|厅|部|机关|单位|公司|中心|站|所|办)(?=[\u4e00-\u9fa5])")
...
# 句子片段而非企业名：「…通过其他方式取得中炬高新技术实业（集团）股份有限公司控制权」
# —— 这类串以组织形式后缀结尾，长度也合法，只能靠里面的**动词**识别。
# 「通过」在企业字号里极罕见，用它做判据误伤面最小。
_VERB_IN = re.compile(r"通过|取得|获得|收购|实施|从事")
...
def org_names(text):
    ...
    for m in _NAME.finditer(t):
        n = m.group(1)
        # 循环清理句首的套话/日期/动词：一次替换切不干净（「本局于2026年9月4日依法对
        # 青岛诺轩建筑材料有限公司」要先切「本」再切「局于…日依法对」）。
        for _ in range(4):
            n2 = _DATE_LEAD.sub("", n)
            n2 = _PREFIX_CUT.sub("", n2)
            n2 = _HALF_ORG.sub("", n2)
            if n2 == n or len(n2) < 4:
                break
            n = n2
        ...
        if len(n) <= 6 and re.match(r"^(?:有限|股份|责任|代理|事务|合伙)", n):
            continue
        if _MASKED.search(n) or _BAD_IN.search(n) or _BAD_ORG.search(n) or _VERB_IN.search(n):
            continue
        if n not in found:
            found.append(n)
...
def extract_subject(fact, title="", limit=3):
    ...
    if name:
        ...
        if _COLLECTION.search(title):
            ...
            if len(others) > limit:
                n = len(others) + 1
        # 「XX公司等 25 家企业（名单附后）」—— 送达公告的标准写法。标题里未必有
        # 「典型案例 / 通报」这类集合词，但正文明确给出了被罚主体的**总数**，
        # 表格里写成「XX公司 等 25 家」比只显示首个企业名准确得多。
        m = re.search(re.escape(name) + r"\s*等\s*(\d{1,4})\s*(?:家|户|个)", fact)
        if m:
