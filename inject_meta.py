...
OG_DESC = {
    ...
    "news/today.html": "今日更新：法规标准增量、生效倒计时、立法节点与草案截止，一屏掌握今天变了什么。"
                       "已并入合规动态模块，与每日监管事件流同源同批。",
    "manage/index.html": "合规管理：把合规要求落成可执行动作——审计范围怎么定、过程怎么留痕、结论怎么变成整改任务。",
    ...
    "kb/standards.html": "个人信息保护、数据安全、网络安全、算法与 AI、移动应用合规，以及网络交易、餐饮外卖、仓储冷链、即时配送、计量、消费者权益与绿色包装等领域的国家标准、法律法规与指引指南汇总，按资源类型、效力级别、时效性、专题、发文机关与地域多维筛选，逐条标注发布与实施日期并附官方原文深链。",
    "kb/duties.html": DUTY_SCALE + "，按「主题大类 × 业务场景」铺开成义务矩阵，每项义务给出可直接引用的法规与标准条款原文、整改优先级、行业标杆做法与可套用文案；还可按业务话题直接问出该话题的法律依据清单。",
    ...
    "analysis/app-violations.html": "移动应用违规治理专项（数据看板）：工业和信息化部与国家网信办历年 App（含小程序、SDK）侵害用户权益与个人信息收集使用问题通报的历史库与数据分析，附可对照自查的合规行动清单。",
    "analysis/algo-filing.html": "算法合规治理专项（数据看板）：国家网信办公告的互联网信息服务算法备案清单、生成式人工智能服务已备案与已登记信息结构化信息库，按算法类别、批次与属地分析。",
    "kb/cases.html": "合规案例库：监管机关官网公开的处罚决定、通报与典型案例，按被处罚主体、处罚事由、违法类型、执法机关、依据法条与罚款幅度结构化索引，逐条附官方原文深链。",
}
