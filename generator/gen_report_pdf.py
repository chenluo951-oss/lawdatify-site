...
UI_TEXTS = [
    ...
    "说明：本板块聚焦与朴朴超市（生鲜电商/前置仓即时零售）业务直接相关的合规动态，逐项评估合规风险等级（高/中/低）并给出应对建议。",
...
def compile_note(S, cutoff=None, n_src=None):
    ...
    rows = [
        Paragraph("覆盖口径：六大合规领域公开合规动态与通报处罚", S["motto"]),
...
SRC_INTERNAL = "朴朴超市法务合规部基于公开合规动态内部评估"
...
def build(all_text_provider):
    ...
    story.append(KeepTogether([fig3_matrix(F, matrix_rows),
                               ...
                               Paragraph("资料来源：朴朴超市法务合规部基于公开合规动态内部评估", S["src"])]))
