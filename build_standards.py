...
def main():
    ...
    body = "\n".join([
        ...
        '<p class="rd-note">跟踪<b>尚未生效</b>的立法与标准制定动态：国标征求意见、部门规章草案、'
        '长期悬置未发布的规范性文件。<b>草案不产生合规义务</b>，但预示监管方向；'
        '剩余天数按打开当天计算。</p>',
        f'<div class="rd-list" id="draftList">{draft_html}</div>',
        "</section>",
        '<p class="rd-note">标准数据取自<b>国家标准全文公开系统</b>，法规与规范性文件均附官网原文深链。'
        '<b>不构成法律意见</b>，引用前请核对现行有效版本。</p>',
        LIB_RENDER,
        TABS_JS,
    ])
    ...
    duty_body = "\n".join([
        ...
        '<p class="rd-note"><b>矩阵总览</b>按「主题大类 × 业务场景」铺开：格内数字＝该场景义务条数，'
        '色点＝整改优先级分布。点格子进入<b>逐条明细</b>，含条款原文、行业标杆做法与可套用文案。</p>',
        duty_html,
        "</section>",
        '<section class="lb-pane" id="pane-ask" hidden>',
        '<p class="rd-note"><b>按话题找依据</b>与义务清单是同一套数据的两个入口：'
        '一个按目录翻，一个按问题问。说清你<b>正在做的事</b>，直接输出该话题的'
        '<b>法律依据清单</b>，可一键复制成 Markdown 或打印成 PDF。</p>',
        ask_html,
