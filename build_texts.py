...
JUNK_LINE = re.compile(
    ...
    r"中华人民共和国中央人民政府|中国政府网|首页|网站首页|\s*)$")
...
def clean(text):
    ...
    # 复制到别处也出错。NFKC 会把它们还原成正规汉字，同时把全角字母数字折半。
...
def match(item, cand):
    ...
    for c in cand:
        ...
        try:
            ...
            continue
        rank = (score, sz, 1 if c["kind"] == "corpus" else 0)
...
def main():
    ...
    for it in laws:
        ...
        try:
            ...
            dropped.append((it.get("name"), "读取失败"))
            continue
        t = clean(raw)
        if len(t) < MIN_CHARS:
            dropped.append((it.get("name"), "篇幅不足 %d 字" % len(t)))
            continue
        # 名称与正文自洽：正文前 600 字含条目名，或含「第X条」条款特征
        core = re.sub(r"[^\u4e00-\u9fa5]", "", (it.get("name") or "").replace("中华人民共和国", ""))[:6]
        head = re.sub(r"[^\u4e00-\u9fa5]", "", t[:600])
        if not ((core and core in head) or ART_RE.search(t)):
            dropped.append((it.get("name"), "正文与该条目不匹配"))
            continue
        qok, why = quality_ok(t)
        if not qok:
            dropped.append((it.get("name"), why))
            continue
